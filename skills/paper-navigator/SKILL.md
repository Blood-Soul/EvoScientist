...
Scripts are available at `/skills/paper-navigator/scripts/`. Run via
...
| `PAPER_NAV_EXPERIENCE_CONCURRENCY` | Batch experience extraction | Max papers extracted at once; defaults to `4` (range `1`–`64`) |
...
|---|---|---|
| URL | `python /skills/paper-navigator/scripts/fetch_paper.py --url <URL> --papers-dir artifacts/papers` | Paper Card + saved full text |
| Title quoted | `python /skills/paper-navigator/scripts/match_paper_by_title.py --title "<title>"` (add `--fallback-search` for typos), then fetch the matched ID | Paper Card + saved full text |
| Bare ID (arXiv / DOI / S2 / CorpusId) | `python /skills/paper-navigator/scripts/fetch_paper.py --paper-id <ID> --papers-dir artifacts/papers` | Paper Card + saved full text |
...
After the full-text path is printed, call the
`extract_paper_experiences` tool with:

- `paper_file`: `<FULL_TEXT_PATH>`
- `paper_id`: `<CANONICAL_PAPER_ID>`

The tool calls both `prompt/l1_extract.md` and `prompt/l2_inductive.md`
concurrently on cache misses. It uses EvoScientist's configured auxiliary model
when present and falls back to the main model otherwise. Parsed L1/L2 JSON is
stored under the current session's isolated paper-experience memory; repeat
reads in the same session reuse it, while other sessions cannot. The tool
returns field-aware readable Markdown—not raw JSON—and that output enters the
agent context. Use those experiences as the primary reading evidence. If full
text or extraction is unavailable, say so and keep the Paper Card; never infer
missing experience content from the abstract.

Stop after the Paper Card plus extracted experiences. Do not chain to citation
...
```bash
python /skills/paper-navigator/scripts/scholar_search.py --query "<Q-broad>"  --limit 15 --sort-by relevance --output /tmp/pool.jsonl --append
python /skills/paper-navigator/scripts/scholar_search.py --query "<Q-narrow>" --limit 15 --sort-by relevance --output /tmp/pool.jsonl --append
```
...
```bash
python /skills/paper-navigator/scripts/scholar_search.py --query "<refine 1: method, angle X>" --limit 15 --output /tmp/pool.jsonl --append
python /skills/paper-navigator/scripts/scholar_search.py --query "<refine 2: method, angle Y>" --limit 15 --output /tmp/pool.jsonl --append
python /skills/paper-navigator/scripts/scholar_search.py --query "<refine 3: lifted entity>"   --limit 15 --output /tmp/pool.jsonl --append
```
...
```bash
python /skills/paper-navigator/scripts/citation_traverse.py --paper-id <SEED> --direction co-citation --limit 15 --output /tmp/pool.jsonl --append
python /skills/paper-navigator/scripts/citation_traverse.py --paper-id <SEED> --direction forward --limit 20 --min-citations 20 --year-min 2022 --output /tmp/pool.jsonl --append
python /skills/paper-navigator/scripts/recommend.py --positive <SEED1>,<SEED2> --limit 15 --output /tmp/pool.jsonl --append
```
...
```bash
python /skills/paper-navigator/scripts/snippet_search.py --query "<criterion phrase>" \
...
### Step 7: Full-text Experience Enrichment

After Step 6 fixes the final ranked set, enrich **every paper that will appear
in the ranked answer**. This means Primary papers, plus a Secondary paper only
when an exact-N request explicitly requires padding. Do not enrich the broad
search pool, WEAK/IRREL candidates, or surplus papers outside the output cap.

For each final paper, fetch the full text, then call the runtime tool:

```bash
python /skills/paper-navigator/scripts/fetch_paper.py \
  --paper-id <PAPER_ID> \
  --papers-dir artifacts/papers
```

Fetch every final paper first. Then call `extract_paper_experiences_batch` once,
with `papers=[{"paper_file": <FULL_TEXT_PATH>, "paper_id": <PAPER_ID>}, ...]`.
It extracts papers concurrently, bounded by `max_concurrency` (or
`PAPER_NAV_EXPERIENCE_CONCURRENCY`, default `4`), and reports
`experience_extraction_elapsed_seconds` for the entire enrichment batch.
Use `extract_paper_experiences` only for a single final paper.

The tool performs the two LLM calls concurrently on cache misses and returns
`L1 Practical Experiences` plus `L2 Inductive Experiences`. These outputs are
part of the working context and must inform the final synthesis. Keep the
ranked metadata/evidence format from Step 6, then add a concise experience
section under each returned paper. Do not paste the full fetched paper into the
final answer. A failure for one paper must not block enrichment of the others;
mark that paper's experience extraction as unavailable and continue.
...
| Extract one paper's experiences | `extract_paper_experiences` tool | Full-text Markdown → parsed/rendered L1 + L2; current-session cache; auxiliary model preferred |
| Extract final papers concurrently | `extract_paper_experiences_batch` tool | Bounded batch enrichment; output includes total extraction elapsed seconds |
